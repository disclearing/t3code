        ...
        function* (input) {
          ...
          const started = yield* Effect.tryPromise({
            try: async () => {
              ...
              if (resumeCursorSessionId) {
                // Attempt to reattach to an existing OpenCode session persisted on disk.
                // The OpenCode server persists sessions, so a fresh server process can
                // resume an existing session by ID as long as the state dir survived.
                try {
                  await client.session.messages({ sessionID: resumeCursorSessionId });
                  return {
                    server,
                    client,
                    openCodeSessionId: resumeCursorSessionId,
                    resumed: true as const,
                  };
                } catch (error) {
                  // Session no longer exists on disk — fall through to create a fresh one
                  // so we don't wedge the thread when state is wiped.
                  console.warn(
                    `[opencode] Failed to resume session '${resumeCursorSessionId}' for thread '${input.threadId}': ${
