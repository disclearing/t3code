...
export function resolveThreadRowClassName(input: {
  ...
  if (input.isSelected && input.isActive) {
    return cn(baseClassName, "bg-primary/30 text-foreground font-medium hover:bg-primary/36");
  }

  if (input.isSelected) {
    return cn(baseClassName, "bg-primary/22 text-foreground hover:bg-primary/28");
  }

  if (input.isActive) {
    return cn(baseClassName, "bg-accent/55 text-foreground font-medium hover:bg-accent/70");
  }
