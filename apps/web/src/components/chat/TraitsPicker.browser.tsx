...
async function mountOpenCodePicker(props?: {
  ...
  const host = document.createElement("div");
  document.body.append(host);
  const openCodeModels = TEST_PROVIDERS.find(
    (provider) => provider.provider === "opencode",
  )?.models;
  if (!openCodeModels) {
    throw new Error("Expected OpenCode models to be present in test providers.");
  }
  const screen = await render(
    <TraitsPicker
      provider="opencode"
...
describe("TraitsPicker (OpenCode)", () => {
  ...
  it("shows both effort and agent controls", async () => {
  ...
  it("persists sticky OpenCode options when selecting an agent", async () => {
    await using _ = await mountOpenCodePicker({ options: { variant: "medium", agent: "build" } });
